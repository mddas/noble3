 ...
 */
class MyTablePrintable_backup implements Printable {
    ...
    /** The font to use in readering sub header text. */
     ...
     */
    public MyTablePrintable_backup(JTable table,
        ...
        headerFont = table.getFont().deriveFont(Font.BOLD,
                                                HEADER_FONT_SIZE);
        SubHeaderFont=table.getFont().deriveFont(Font.ITALIC,
                SubHeader_FONT_SIZE);
        ...
        if (headerText != null) {
            for(int i=0 ; i<headerText.length ; i++)
                if (i<=6){
                    ...
                    printText(g2d, headerText[i], hRect[i], SubHeaderFont, i, imgWidth);
                }
